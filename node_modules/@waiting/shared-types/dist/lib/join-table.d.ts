...
export type JoinTableWithCaseConvert<T1 extends {}, T2 extends {}, Prefix extends string, CaseConvert extends CaseType> = CaseConvertTable<JoinTable<T1, T2, Prefix>, CaseConvert>;
export type CaseConvertTable<T extends {}, CaseConvert extends CaseType> = CaseConvert extends CaseType.camel ? RecordCamelKeys<T> : CaseConvert extends CaseType.snake ? RecordSnakeKeys<T> : CaseConvert extends CaseType.pascal ? RecordPascalKeys<T> : T;
export type JoinTable<T1 extends {}, T2 extends {}, Prefix extends string> = T1 & _JoinCover<T1, T2, Prefix> & _JoinDiff<T1, T2>;
 ...
 */
export type DbScopedColsByKey<D extends {}, Tb extends keyof D = keyof D> = D extends Record<infer F extends (Tb extends string ? Tb : string), unknown> ? F extends unknown ? `${F}.${StrKey<D[F]>}` : never : never;
/**
 * Create scoped column name from all tables of D,
 * filter by Tb Type
 */
