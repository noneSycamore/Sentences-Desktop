...
Napi::Value Attach(const Napi::CallbackInfo &info)
{
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        { // 以有效字符比对，防止连同字符“0”等无效字符也一同包含在一起比对
            childWindow_hwnd = FindWindowExA(class_name->win_hwnd, 0, "TMainBox", NULL);
            if (childWindow_hwnd != NULL)
            {
                insert_hWnd = childWindow_hwnd;
                break;
            }
            SHELLDLL_hwnd = FindWindowExA(class_name->win_hwnd, 0, "SHELLDLL_DefView", NULL);
            if (SHELLDLL_hwnd != NULL)
            {
                record_hWnd = class_name->win_hwnd;
            }
        }
