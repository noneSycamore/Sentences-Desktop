...
import { dllName, ref, retriveStruct_PRINTER_INFO, retriveStruct_PRINTPROCESSOR_INFO_1, retriveStruct_DATATYPES_INFO_1, } from './helper.js';
export async function winspoolClosePrinter(hPrinter) {
    assert(hPrinter);
 ...
 */
export async function winspoolEnumPrinters(options) {
    const mod = getMod(dllName);
 ...
 */
export async function winspoolEnumPrintProcessors(pName, pEnvironment) {
    const mod = getMod(dllName);
...
export async function winspoolEnumPrintProcessorDatatypes(pName, pPrintProcessorName) {
    const mod = getMod(dllName);
    const pNameBuf = ucsBufferFrom(pName);
 ...
 */
export async function winspoolGetDefaultPrinter(maxNameLength = 256) {
    const mod = getMod(dllName);
 ...
 */
export async function winspoolOpenPrinter(printerName) {
    const mod = getMod(dllName);
    assert(printerName);
 ...
 */
export async function winspoolGetPrinter(hPrinter, Level, maxByteLength = 1024) {
    const mod = getMod(dllName);
...
export async function winspoolStartDocPrinter(hPrinter, pDocInfo) {
    const mod = getMod(dllName);
    assert(hPrinter);
    assert(pDocInfo);
    const Level = 1;
...
export async function winspoolStartPagePrinter(hPrinter) {
    const mod = getMod(dllName);
    assert(hPrinter);
    const ret = await mod.StartPagePrinter(hPrinter.toString());
    return ret;
}
export async function winspoolEndPagePrinter(hPrinter) {
    const mod = getMod(dllName);
    assert(hPrinter);
    const ret = await mod.EndPagePrinter(hPrinter.toString());
    return ret;
}
//# sourceMappingURL=index.winspool.js.map